...
var excludes = '--exclude=.git --exclude=node_modules --exclude=package.json --exclude=.*';
...
function diffGitAndNpm(pkg, version, cb) {
  ...
  var npmSubDir = npmDir.substring(prefix.length);

  var cmd = '';
  cmd += util.format('cd %s', prefix);
  cmd += util.format(' && diff -aur %s %s %s | diffstat -q',
                     excludes, gitSubDir, npmSubDir);
  child_process.exec(cmd, function (err, stdout, stderr) {
    if (err && err.signal) return cb(err);
    if (err && err.code !== 1) return cb(err);
    if (stderr.length !== 0) {
      return cb(new Error('stderr had something'), 'stderr\n' + stdout);
    }
    if (stdout.length === 0) {
    ...
    // We found a diff.
    log.warn('Found a diff (%s@%s), %d', pkg, version, stdout.length);
    cb(null, stdout);
  });
}
